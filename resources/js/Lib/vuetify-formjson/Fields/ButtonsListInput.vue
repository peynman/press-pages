  ...
  <div :class="`vf-input ${field.class ? field.class:''}`">
    ...
    <v-tabs v-else>
      <v-tab
        ...
        :key="`${id}-buttons-list-group-${group.id}`"
        :href="`#${id}-buttons-${group.id}`"
      >
        <label>{{ group.title }}</label>
      </v-tab>
      <v-tab-item
        v-for="group in field.groups"
        :key="`${id}-buttons-list-group-item-${group.id}`"
        :value="`${id}-buttons-${group.id}`"
      >
        <v-btn
          v-for="(btn, index) in field.objects"
          v-if="btn.group === group.id"
          :key="`${id}-buttons-list-${index}`"
          :class="`vf-input vf-btn ${field.props && field.props.class ? field.props.class : ''} ${btn.class ? btn.class:''}`"
          @click="onButtonClick(index)"
          :color="getButtonClass(index)"
...
export default {
  ...
  methods: {
    ...
    getButtonClass(index) {
      if (this.devalue.indexOf(index) >= 0) {
        return this.field.activeClass
          ? this.field.activeClass
          : this.field.objects &&
            this.field.objects[index] &&
            this.field.objects[index].activeClass
          ? this.field.objects[index].activeClass
          : "primary";
      } else {
        return this.field.normalClass
          ? this.field.normalClass
          : this.field.objects &&
            this.field.objects[index] &&
            this.field.objects[index].normalClass
          ? this.field.objects[index].normalClass
          : "";
      }
    },
