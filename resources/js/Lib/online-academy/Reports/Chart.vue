<template>
  ...
  >
    <v-card-actions dark class="text-subtitle-1 pt-0">
      <v-list dense class="px-0 mx-auto">
        <v-list-item class="px-0">
          <v-list-item-content>
            <v-list-item-title>{{ field.label }}</v-list-item-title>
            <v-list-item-subtitle v-if="field.subtitle">
              {{ field.subtitle }}
              <v-chip dense small>{{ this.duration }}</v-chip>
              <v-chip dense small v-if="field.is_windowed">{{ this.window }}</v-chip>
            </v-list-item-subtitle>
          </v-list-item-content>
          <v-list-item-action class="my-auto d-flex flex-row">
            <v-btn :loading="loading" icon small @click="updateReport()">
      ...
      <div v-show="show">
        ...
        <v-card-text class="d-flex flex-column">
          ...
          <v-select
            ...
            item-value="id"
          ></v-select>
          <vf-fields-renderer v-if="field.filters" v-model="extraFilters" :fields="field.filters" :options="{ type: 'col', }"></vf-fields-renderer>
        </v-card-text>
      </div>
    </v-expand-transition>
    <v-card-text class="pb-0">
      <canvas ref="chart" style="height: 80%;"></canvas>
    </v-card-text>
  </v-card>
</template>
